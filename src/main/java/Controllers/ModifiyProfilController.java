...
// Vérifier si le mot de passe et sa confirmation correspondent et afficher une erreur si ce n'est pas le cas
    ...
    void ModifiyUser(ActionEvent event) {
       ...
       // Role type = Type.getValue(); // Get the selected item from ComboBox
        ...
        try {
            ...
            User updatedUser = new User(this.userId,email,password,nom,prenom,addres,Num_tel,confirmPass,dateJoined,imageData);
            updatedUser.setRole(User.Role.valueOf("CLIENT"));
    ...
    private void loadData() {
        try {
            User user =serviceUser.findById(GuiLoginController.user.getId());
            if (user != null) {
