...
public class ModifyUserController implements Initializable,InitializableController {
    private int userId;
  ...
  // @Override
   // public void initialize(URL url, ResourceBundle resourceBundle) {
     //   Type.getItems().addAll(Users.Roles.values());
       // loadData();
       // btnReturn.setOnMouseClicked(new EventHandler<MouseEvent>() {
        //    @Override
          //  public void handle(MouseEvent event) {
          //      System.out.println("Returning to users CRUD");
           //     RouterController.navigate("/fxml/Admin/UsersCRUD.fxml");
         //   }
     //   });
  //  }
   @Override
   public void initialize(URL url, ResourceBundle resourceBundle) {
     ObservableList<String> roles = FXCollections.observableArrayList("[\"ROLE_ADMIN\"]", "[\"ROLE_CLIENT\"]", "[\"ROLE_VET\"]", "[\"ROLE_INSTRUCTOR\"]");
     Type.setItems(roles);
     loadData();
     btnReturn.setOnMouseClicked(new EventHandler<MouseEvent>() {
       @Override
...
// Vérifier si le mot de passe et sa confirmation correspondent et afficher une erreur si ce n'est pas le cas
    ...
    void ModifiyUser(ActionEvent event) {
            ...
            // Users.Roles type = Type.getValue(); // Get the selected item from ComboBox
            java.util.Date dateJoined = java.sql.Date.valueOf(Date.getValue());
            System.out.println(nom+prenom);
        try {
            // Update the activity
         //Roles roles = Roles.valueOf(type.substring(1, type.length() - 1)); // Convertir la chaîne de rôle en rôle approprié

                  //  Users updatedUser = new Users(this.userId, email, password, nom, prenom, addres, Num_tel, confirmPass, dateJoined, roles, imageData);
          Users updatedUser = new Users(this.userId,email,password,nom,prenom,addres,Num_tel,confirmPass,dateJoined,type,imageData);
    ...
    private void loadData() {
        try {
            Users user =serviceUser .findById(userId);
            if (user != null) {
                System.out.println(user);
                Nom.setText(user.getName());
             // String roleString = user.getRoles().getRoleName();
             // Type.setValue(roleString);
              Type.setValue(user.getRoles());
               // Type.setValue(Users.Roles.valueOf(user.getRoles().name()));
    ...
    @Override
    public void init(Integer Id) throws SQLException {
this.userId=Id;
loadData();
    }
}
