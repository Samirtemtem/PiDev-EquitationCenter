...
public class ServiceUser implements IService<User> {
    ...
    public void add(User user) throws SQLException {
        ...
        String query = "INSERT INTO users (email, hash,salt, name,lastname,address,num_tel,dateJoined, role,ImageData) VALUES (?,?, ?, ?, ?,?,?,?,?,?)";
    ...
    public void update(User user) throws SQLException {
        String query = "UPDATE users SET email = ?, password = ?, name = ?, lastname = ?,address = ?,num_tel = ?,dateJoined = ?,role = ? ,ImageData = ? WHERE id = ?";
        try (PreparedStatement statement = connection.prepareStatement(query)) {
            statement.setString(1, user.getEmail());
