...
public class ServiceHorse implements IService<Horse> {
    ...
    public List<Horse> ReadAvailableHorses() throws SQLException {
        List<Horse> horses = new ArrayList<>();
        String query = "SELECT * FROM horse WHERE isAvailable=1";
        try (PreparedStatement stmt = conn.prepareStatement(query);
             ResultSet rs = stmt.executeQuery()) {
            while (rs.next()) {
                Horse horse = createHorseFromResultSet(rs);
